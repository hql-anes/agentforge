    ...
    <main className="min-h-screen overflow-hidden bg-[#060914] text-white">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold tracking-wide">AgentForge Workflow</div>
            <div className="text-xs text-white/45">Multi-Agent R&D Automation Platform</div>
          </div>
        </div>
        <div className="hidden items-center gap-6 text-sm text-white/60 md:flex">
          <a href="#architecture" className="hover:text-white">架构</a>
          <a href="#backend" className="hover:text-white">后端</a>
          <a href="#workflow" className="hover:text-white">工作流</a>
          <a href="#evidence" className="hover:text-white">项目材料</a>
        </div>
        <Button className="rounded-full bg-white text-black hover:bg-cyan-100">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-20 pt-14 lg:grid-cols-[1.05fr_0.95fr]">
        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-cyan-200 shadow-lg backdrop-blur">
            <Sparkles className="h-4 w-4" />
            AI Native · Agent 编排 · 自动验证 · 成本评测
          </div>
          <h1 className="max-w-4xl text-5xl font-semibold leading-tight tracking-[-0.04em] md:text-7xl">
            让多 Agent 协作
            <span className="bg-gradient-to-r from-cyan-300 via-blue-300 to-violet-300 bg-clip-text text-transparent"> 自动完成复杂 AI 研发任务</span>
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-white/62">
            AgentForge 将需求理解、任务规划、工具调用、代码生成、测试验证、质量评审和成本分析串成闭环，帮助个人开发者与中小型技术团队把 AI 从“聊天辅助”升级为“可追踪、可评测、可复用”的自动化研发工作流。
          </p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button size="lg" className="rounded-full bg-cyan-300 px-7 text-black hover:bg-cyan-200">
              查看在线演示 <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7 }} className="relative">
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-4 shadow-2xl backdrop-blur-xl">
            <div className="rounded-[1.5rem] border border-white/10 bg-[#090F1E] p-5">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <div className="text-sm text-white/45">Live Workflow Trace</div>
                  <div className="font-medium">AI R&D Demo Pipeline</div>
                </div>
                <div className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs text-emerald-300 ring-1 ring-emerald-400/20">Running</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4 font-mono text-xs leading-7 text-white/72">
                {logs.map((log, index) => (
                  <div key={log} className="flex gap-3">
                    <span className="text-white/30">{String(index + 1).padStart(2, "0")}</span>
                    <span className={index === logs.length - 1 ? "text-emerald-300" : ""}>{log}</span>
                  </div>
                ))}
              </div>
      ...
      <section id="architecture" className="mx-auto max-w-7xl px-6 py-20">
        <div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <div className="mb-3 text-sm uppercase tracking-[0.25em] text-cyan-300/80">Layered Agent Architecture</div>
            <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">六类 Agent 构成研发闭环</h2>
          </div>
          <p className="max-w-xl text-white/55">系统不是单一聊天机器人，而是一个面向长链路任务的 Agent 编排平台，所有执行过程都可追踪、可复盘、可评测。</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {agents.map((agent) => {
            const Icon = agent.icon;
            return (
              <Card key={agent.name} className="rounded-3xl border-white/10 bg-white/[0.045] text-white shadow-xl backdrop-blur">
                <CardContent className="p-6">
                  <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/8 ring-1 ring-white/10">
                    <Icon className="h-6 w-6 text-cyan-300" />
                  </div>
                  <h3 className="text-xl font-semibold">{agent.name}</h3>
                  <p className="mt-3 leading-7 text-white/55">{agent.desc}</p>
                </CardContent>
              </Card>
            );
          })}
      ...
      <section id="backend" className="mx-auto max-w-7xl px-6 py-20">
        <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[0.08] to-white/[0.025] p-8 shadow-2xl backdrop-blur-xl md:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr]">
            <div>
              <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-violet-300/10 ring-1 ring-violet-300/20">
                <Server className="h-6 w-6 text-violet-200" />
              </div>
              <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">后端编排与执行引擎</h2>
              <p className="mt-5 leading-8 text-white/58">
                推荐采用 FastAPI + PostgreSQL + Redis + Celery/RQ 的组合，将 Agent 调度、任务状态、工具调用、日志追踪与成本统计统一封装为后端服务。前端只负责创建任务、查看实时进度与下载报告。
              </p>
              <div className="mt-6 flex flex-wrap gap-2">
                {['FastAPI', 'PostgreSQL', 'Redis Queue', 'RAG', 'Structured Output', 'Token Cost'].map((tag) => (
                  <span key={tag} className="rounded-full border border-white/10 bg-black/25 px-3 py-1 text-sm text-white/62">{tag}</span>
                ))}
              </div>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {backendModules.map(([Icon, title, desc]) => (
      ...
      <section id="workflow" className="mx-auto max-w-7xl px-6 py-20">
        <div className="mb-10 text-center">
          <div className="mb-3 text-sm uppercase tracking-[0.25em] text-cyan-300/80">Execution Pipeline</div>
          <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">从需求到结果的自动化链路</h2>
        </div>
        <div className="grid gap-4 lg:grid-cols-5">
          {workflow.map((item) => (
            <div key={item.step} className="relative rounded-3xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur">
              <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-2xl bg-cyan-300/10 text-sm font-semibold text-cyan-200 ring-1 ring-cyan-300/20">{item.step}</div>
      ...
      <section id="evidence" className="mx-auto max-w-7xl px-6 py-20">
        <div className="rounded-[2rem] border border-cyan-300/15 bg-[#081120]/80 p-8 shadow-2xl backdrop-blur md:p-10">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">项目证明材料已整理</h2>
              <p className="mt-5 leading-8 text-white/58">
                页面不再展示“占位地址”。这里改为正式的项目材料入口，用于放置真实部署后的 GitHub、在线演示、运行日志、评测报告与账单截图。提交前只需要替换为你的真实链接即可。
              </p>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              {[
      ...
      <section className="mx-auto max-w-7xl px-6 pb-24 pt-8">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-8 text-center shadow-2xl backdrop-blur md:p-12">
          <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-3xl bg-cyan-300/10 ring-1 ring-cyan-300/20">
            <Zap className="h-7 w-7 text-cyan-300" />
          </div>
          <h2 className="text-3xl font-semibold tracking-tight md:text-5xl">把复杂 AI 任务沉淀为可复用工作流</h2>
          <p className="mx-auto mt-5 max-w-2xl leading-8 text-white/58">
            适用于自动化研发 Demo、批量模型评测、代码与内容生成测试、技术文章生成、RAG 工作流和运营自动化场景。
          </p>
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Button size="lg" className="rounded-full bg-cyan-300 px-8 text-black hover:bg-cyan-200">Start Workflow</Button>
            <Button size="lg" variant="outline" className="rounded-full border-white/15 bg-white/5 px-8 text-white hover:bg-white/10">Read Docs</Button>
          </div>
        </div>
      </section>
    </main>
